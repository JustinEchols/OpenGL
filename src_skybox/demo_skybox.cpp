		...
		mesh_textures_t *MeshTextures, u32 texture_count, aiTextureType texture_type, char *path_to_dir)
{
	...
	// code. The path to the texture keeps getting overwritten. What is a better
	// approach. Creating a local texture everytime in the loop, or getting a
	// pointer to the array of textures and filling out a single texture then
		...
		char *path_to_dir, char *vertex_shader_filename, char *geometry_shader_filename, char *fragment_shader_filename)
{
	...
	for (u32 i = 0; i < Node->mNumMeshes; i++)
	{
		...
		MeshShader = shader_program_create_from_files(vertex_shader_filename, fragment_shader_filename);
#if 1
		...
		char *vertex_shader_filename, char *geometry_shader_filename, char *fragment_shader_filename)
{
	...
	node_process(AppState, Scene, Node, &Result, PathToDir.data, 
			vertex_shader_filename, geometry_shader_filename, fragment_shader_filename);
...
int main(void)
{
	...
	model_process(&AppState, "models/suzanne.obj", "shaders/environment_map.vs", NULL, "shaders/environment_map.fs");

	shader_program_t EnvShader = shader_program_create_from_files("shaders/environment_map.vs", "shaders/environment_map.fs");
	...
	cube_t CubeMirror = cube_no_textures_init(&cube_vertices_and_normals[0],
							ArrayCount(cube_vertices_and_normals),
...
#if 0
	model_t ModelSuzanne = AppState.Models[0];
	for(u32 mesh_index = 0; mesh_index < ModelSuzanne.mesh_count; mesh_index++)
	{
		mesh_t *ModelMesh = ModelSuzanne.Meshes + mesh_index;
		ModelMesh->MeshTextures.Textures[ModelMesh->MeshTextures.texture_count++] = SkyBox.TextureCubeMap;
		ModelMesh->MaterialType = Reflective;
	}
#endif
	...
	while (!glfwWindowShouldClose(Window.handle))
	{
		...
		//
		// NOTE(Justin): Skybox textures on local cube.
		//
		...
		//
		// NOTE(Justin): Reflective cube.
		//

		glUseProgram(CubeMirror.Shader.id);

		ModelTransform = glm::translate(ModelTransform, E1);

		uniform_set_mat4f(CubeMirror.Shader.id, "u_ModelTransform", ModelTransform);
		uniform_set_mat4f(CubeMirror.Shader.id, "u_MapToCamera", MapToCamera);
		uniform_set_mat4f(CubeMirror.Shader.id,"u_MapToPersp", MapToPersp);

		uniform_set_vec3f(CubeMirror.Shader.id,"u_CameraPos", AppState.Camera.Pos);
		...
		//
		// NOTE(Justin): Reflective Suzanne model.
		//
		
		glUseProgram(EnvShader.id);
		ModelTransform = glm::translate(ModelTransform, 2 * E2 - E1);
		ModelTransform = glm::rotate(ModelTransform, (f32)glfwGetTime(), E1 + E2 + E3);

		uniform_set_mat4f(EnvShader.id, "u_ModelTransform", ModelTransform);
		uniform_set_mat4f(EnvShader.id, "u_MapToCamera", MapToCamera);
		uniform_set_mat4f(EnvShader.id,"u_MapToPersp", MapToPersp);
		uniform_set_vec3f(EnvShader.id,"u_CameraPos", AppState.Camera.Pos);
		...
		for(u32 model_index = 0; model_index < AppState.model_count; model_index++)
		{
			...
			for(u32 mesh_index = 0; mesh_index < Model.mesh_count; mesh_index++)
			{
				...
				mesh_draw2(&ModelMesh, &EnvShader);
			}
		}
