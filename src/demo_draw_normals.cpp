...
int main(void)
{
	...
	model_process(&AppState, "models/backpack/backpack.obj", "shaders/draw_normals.vs", "shaders/draw_normals.gs", "shaders/draw_normals.fs");

	//
	// NOTE(Justin): Geometry shader examples
	//

	shader_program_t Backpack = shader_program_create_from_files("shaders/default.vs", "shaders/default.fs");
	gl_log_shader_info(&Backpack);

	shader_program_t DrawNormals = shader_program_create_from_file(GL_VERTEX_SHADER, "shaders/draw_normals.vs");
	shader_program_add_shader(&DrawNormals, GL_GEOMETRY_SHADER, "shaders/draw_normals.gs");
	shader_program_add_shader(&DrawNormals, GL_FRAGMENT_SHADER, "shaders/draw_normals.fs");

	gl_log_shader_info(&DrawNormals);
	...
	while (!glfwWindowShouldClose(Window.handle))
	{
		...
		glUseProgram(Backpack.id);

		glm::mat4 ModelTransform = glm::mat4(1.0f);
		ModelTransform = glm::translate(ModelTransform, -E3 );
		MapToCamera = glm::lookAt(AppState.Camera.Pos, AppState.Camera.Pos + AppState.Camera.Direction, AppState.Camera.Up);

		uniform_set_mat4f(Backpack.id, "u_ModelTransform", ModelTransform);
		uniform_set_mat4f(Backpack.id, "u_MapToCamera", MapToCamera);
		uniform_set_mat4f(Backpack.id,"u_MapToPersp", MapToPersp);
		...
		for(u32 model_index = 0; model_index < AppState.model_count; model_index++)
		{
			...
			for(u32 mesh_index = 0; mesh_index < Model.mesh_count; mesh_index++)
			{
				...
				mesh_draw2(&ModelMesh, &Backpack);
			}
		}

		glUseProgram(DrawNormals.id);

		uniform_set_mat4f(DrawNormals.id, "u_ModelTransform", ModelTransform);
		uniform_set_mat4f(DrawNormals.id, "u_MapToCamera", MapToCamera);
		uniform_set_mat4f(DrawNormals.id,"u_MapToPersp", MapToPersp);

		// NOTE(Justin): We set the uniform transforms above and are using that
		// value here too. Since we want to draw the model twice using the same
		// transforms, the difference between the draw calls is the bound shader
		// program. 
		...
		for(u32 model_index = 0; model_index < AppState.model_count; model_index++)
		{
			...
			{
				mesh_t ModelMesh = Model.Meshes[mesh_index];
				mesh_draw2(&ModelMesh, &DrawNormals);
			}
		}
