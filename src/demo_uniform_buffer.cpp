...
int main(void)
{
	...
	// NOTE(Justin): The shaders and textures are irrevelant here. We are
	// coloring each cube a solid color using a fragment shader for each cube
	...
	// NOTE(Justin): Now that the projection transform has been defined, buffer
	// the data.
	...
	while (!glfwWindowShouldClose(Window.handle))
	{
		...
		// Note(Justin): We need to buffer the camera matrix every frame becuase
		// the camera pos and dir change frequently. If we do not use the ubo
		// technique we would be setting the MapToCamera uniform for each
		// shader, every frame. Instead it is buffered and each shader has
		// access to this buffer and hence the camera transform.
